import { buildBackend, buildFrontend } from '../assets/buildCommand/buildUtils.js';
import { copyBackend, copyFrontend } from '../assets/buildCommand/copyUtils.js';
import { generateManagerFiles } from '../assets/buildCommand/generateManagerFilesUtils.js';
import { updateNpmPackage } from '../assets/buildCommand/npmPackageUtils.js';
import { postCopyBackendScript, postCopyFrontendScript } from '../assets/buildCommand/postCopyUtils.js';
const buildCommand = {
    name: 'build',
    scope: 'in',
    ...
    run: async (toolbox, options, args, command) => {
        const { print: { infoLoader, warn }, fileSystem: { exists, removeAsync }, prompts, strings: { kebabCase }, path, } = toolbox;
        const { backend, frontend } = toolbox.project;
        ...
        function validateHostname(hostname) {
            const pattern = /(\b(?:(?:2(?:[0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9])\.){3}(?:(?:2([0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9]))\b)|localhost|(^((?:([a-z0-9]\.|[a-z0-9][a-z0-9\-]{0,61}[a-z0-9])\.)+)([a-z0-9]{2,63}|(?:[a-z0-9][a-z0-9\-]{0,61}[a-z0-9]))\.?$)/gi;
