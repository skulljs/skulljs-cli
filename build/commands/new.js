...
const newCommand = {
    ...
    run: async (toolbox, options, args, command) => {
        const { system: { run }, fileSystem: { exists, removeAsync, cwd, readAsync }, print: { infoLoader, error, warn }, saveLog: { generate, copy }, exit, prompts, strings: { kebabCase }, meta: { version }, path, } = toolbox;
        ...
        // choose repository
        let frontend_repo_selected;
        let backend_repo_selected;
        if (single) {
            if (front) {
        ...
        if (frontend_repo_selected) {
            ...
            if (!isFrontendRepoDownloaded)
                exit(command, ['Unable to get the frontend repository.', `Url: ${frontend_repo_selected.url}`]);
            const frontend_package = await readAsync(path.join(frontend_repo_dir, 'package.json'));
        ...
        if (backend_repo_selected) {
            ...
            if (!isBackendRepoDownloaded)
                exit(command, ['Unable to get the backend repository.', `Url: ${backend_repo_selected.url}`]);
            const backend_package = await readAsync(path.join(backend_repo_dir, 'package.json'));
