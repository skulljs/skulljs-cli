import { Option, Argument, Command as Cmd } from 'commander';
import { readdirSync } from 'node:fs';
import * as url from 'url';
import path from 'path';
import toolbox from './toolbox/toolbox.js';
import scopeExt from './runtime/scope-check.js';
import errorsHandler from './runtime/exit-handler.js';
import setCommand from './runtime/set-command.js';
...
async function run(argv, options) {
    ...
    const commands = await Promise.all(commandsPath.map(async (path) => {
        return (await import(`./commands/${path}`)).default;
    }));
    // Add all cli commands from repositories
    repositories.backend_repositories.forEach((repo) => {
        if (repo.cli)
            commands.push(generateCliCmd(repo.cli));
    });
