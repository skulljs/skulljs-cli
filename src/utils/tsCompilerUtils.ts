 ...
 */
async function transformAndWrite(sourceFile: TransformFile, transformers: ts.TransformerFactory<ts.SourceFile>[], saveLog: boolean = false): Promise<void> {
 ...
 */
function getTsProgram(files: ProgramFile[]): ProgramResult {
  const filesPath = files.map((file) => file.path);

  const fileNotExists = filesPath.some((path) => {
    if (!toolbox.fileSystem.exists(path)) return true;
    return false;
  });

  if (fileNotExists) toolbox.exit(toolbox.command, `Files do not exist in ${filesPath}`);
  ...
  if (sourcesNotExists) toolbox.exit(toolbox.command, `Sources do not exist in ${Object.values(sourceFiles)}`);
